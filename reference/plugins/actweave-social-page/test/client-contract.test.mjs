...
test('social workbench is a prompt-only DSH conversation view', async () => {
  ...
  assert.match(source, /accountLists/u)
  assert.match(source, /请说出你要查询的内容/u)
  assert.match(source, /去 Chat 看/u)
  assert.doesNotMatch(source, /SessionResult/u)
  assert.doesNotMatch(source, /查看运行记录/u)
  assert.match(source, /<social_account_json>/u)
...
test('workbench flows stop before every live side effect', async () => {
  ...
  assert.match(source, /never an embedded or in-app browser pane/u)
  assert.match(source, /never a separate, fresh, or headless browser profile/u)
  assert.doesNotMatch(source, /target: '_blank'/u)
  assert.doesNotMatch(source, /actweave-social__link/u)
})
